...
#fits a discrete time Gompertz with covariate. The covariate is averaged over a range of timescales

alpha.vec <- 365/exp(seq(log(7), log(2*365), length.out=64)) #vector of timescales to examine from 7 days to 2 years (730 days). reduce length.out to 


#fits the discrete Gompertz model to discretely sampled data.
#x is a vector of population abundances
Gomp.fit <- function(x, env.state) {

  if(any(is.na(x))) { stop() } #check to see if all abundances are there.

  xcov    <- log(x[-length(x)])#this will be the lagged predictor
  pgr     <- log(x[-1]/x[-length(x)])#the population growth rate (response variable)
  
  delta.vec <- 1/seq(7K, 2*365, by=1) #timescales to average the environmental covariate.
...
#plot the output
plot(365/alpha.vec, rsq.gomp, ylim=c(0,1))
lines(365/alpha.vec, rsq.mean, col="red")
lines(365/alpha.vec, rsq.decay, col="cornflowerblue")
